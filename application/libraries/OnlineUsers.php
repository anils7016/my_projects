...
function set_data($data=false, $force_update=false){
	if(!is_array($data)){ return false;} $tmp=false;
	...
	if(!$tmp) return false; return $this->_save();
} 
// 
function get_info(){ 
	return @$this->data;
...
function _save() { $fp = fopen($this->file,'w'); flock($fp, LOCK_EX); $write = fwrite($fp, serialize($this->data)); fclose($fp); return $write; } }\
?>